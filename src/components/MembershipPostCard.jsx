  ...
  return (
    ...
    >
      ...
      >
        {post.cover?.url ? (
          <div
            className={`membership-post-card__cover ${
              blurredPreview ? 'membership-post-card__cover--blurred' : ''
            }`}
          >
            <img
              src={membershipMediaUrl(post.cover.url)}
              alt={blurredPreview ? '' : post.cover.altText || ''}
              loading={featured ? 'eager' : 'lazy'}
            />
            {blurredPreview ? (
              <span className="membership-post-card__cover-lock">
          ...
          <div
            className="membership-post-card__cover membership-post-card__cover--empty"
            aria-hidden="true"
          >
      ...
      {!post.locked && onSave ? (
        <button
          ...
          aria-label={
            post.isSaved
              ? `Odstrániť ${post.title} z uložených`
              : `Uložiť ${post.title}`
          }
        >
          <Bookmark
